...
run-megabot: ## run the megabot dev ( default )
	@yarn -s run-megabot

run-megabot-dev: ## run-megabot-dev - local
	@yarn -s run-megabot-dev

run-megabot-prod: ## run-megabot-dev - real server ( gpu server )
	@yarn -s run-megabot-prod		
...
build-ra-language-korean:
	@echo "Transpiling ra-language-korean files...";
	@cd ./packages/ra-language-korean && yarn -s build
...
build-data-generator:
	...
	@cd ./megabot_client/data-generator && yarn -s build

build: build-ra-core build-ra-ui-materialui build-ra-data-fakerest build-ra-data-json-server build-ra-data-simple-rest build-ra-data-graphql build-ra-data-graphcool build-ra-data-graphql-simple build-ra-i18n-polyglot build-ra-input-rich-text build-data-generator build-ra-language-english build-ra-language-french build-ra-language-korean build-react-admin  ## compile ES6 files to JS

build-api-pack : build-ra-core build-ra-data-simple-rest
